    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            if mailing.status != 'Запущена' and mailing.start_time <= timezone.now() <= mailing.end_time:
                ...
                mailing.send_emails()
                self.stdout.write(self.style.SUCCESS(f'Рассылка {mailing_id} успешно выполнена.'))
            else:
                self.stderr.write(f'Рассылка {mailing_id} уже запущена или время не подходит.')
